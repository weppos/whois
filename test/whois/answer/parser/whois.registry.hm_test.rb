require 'test_helper'
require 'whois/answer/parser/whois.registry.hm'

class AnswerParserWhoisRegistryHmTest < Whois::Answer::Parser::TestCase

  def setup
    @klass  = Whois::Answer::Parser::WhoisRegistryHm
  ...
  def test_created_on
    parser    = @klass.new(load_part('/registered.txt'))
    expected  = Time.parse("2003-04-28")
  ...
  def test_expires_on
    parser    = @klass.new(load_part('/registered.txt'))
    expected  = Time.parse("2020-04-28")
