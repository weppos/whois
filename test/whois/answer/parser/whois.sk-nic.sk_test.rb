require 'test_helper'
require 'whois/answer/parser/whois.sk-nic.sk'

class AnswerParserWhoisSkNicSkTest < Whois::Answer::Parser::TestCase

  def setup
    @klass  = Whois::Answer::Parser::WhoisSkNicSk
  ...
  def test_updated_on
    parser    = @klass.new(load_part('registered.txt'))
    expected  = Time.parse("2010-06-28")
  ...
  def test_expires_on
    parser    = @klass.new(load_part('registered.txt'))
    expected  = Time.parse("2011-07-23")
