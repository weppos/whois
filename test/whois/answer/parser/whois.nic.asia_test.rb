...
class AnswerParserWhoisNicAsiaTest < Whois::Answer::Parser::TestCase
  ...
  def test_created_on
    parser    = @klass.new(load_part('status_registered.txt'))
    expected  = Time.parse("2008-01-15 11:28:02 UTC")
  ...
  def test_updated_on
    parser    = @klass.new(load_part('status_registered.txt'))
    expected  = Time.parse("2008-03-16 04:30:25 UTC")
  ...
  def test_expires_on
    parser    = @klass.new(load_part('status_registered.txt'))
    expected  = Time.parse("2010-01-15 11:28:02 UTC")
  ...
  def test_nameservers
    parser    = @klass.new(load_part('status_registered.txt'))
    expected  = %w( ns1.dotasia.org ns2.dotasia.org ).map { |ns| nameserver(ns) }
