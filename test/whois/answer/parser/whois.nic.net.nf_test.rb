require 'test_helper'
require 'whois/answer/parser/whois.nic.net.nf'

class AnswerParserWhoisNicNetNfTest < Whois::Answer::Parser::TestCase

  def setup
    @klass  = Whois::Answer::Parser::WhoisNicNetNf
  ...
  def test_created_on
    parser    = @klass.new(load_part('registered.txt'))
    expected  = Time.parse("2005-11-27")
  ...
  def test_updated_on
    parser    = @klass.new(load_part('registered.txt'))
    expected  = Time.parse("2010-03-19")
