require 'test_helper'
require 'whois/answer/parser/whois.nic.ci'

class AnswerParserWhoisNicCiTest < Whois::Answer::Parser::TestCase

  def setup
    @klass  = Whois::Answer::Parser::WhoisNicCi
    @host   = "whois.nic.ci"
  end
  ...
  def test_created_on
    parser    = @klass.new(load_part('status_registered.txt'))
    expected  = Time.parse("2006-01-27 11:14:47.77")
  ...
  def test_expires_on
    parser    = @klass.new(load_part('status_registered.txt'))
    expected  = Time.parse("2008-01-29 11:14:47.77")
