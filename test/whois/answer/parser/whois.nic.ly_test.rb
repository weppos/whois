require 'test_helper'
require 'whois/answer/parser/whois.nic.ly'
...
class AnswerParserWhoisNicLyTest < Whois::Answer::Parser::TestCase
  ...
  def test_created_on
    parser    = @klass.new(load_part('registered.txt'))
    expected  = Time.parse("2007-10-03 13:36:48")
  ...
  def test_updated_on
    parser    = @klass.new(load_part('registered.txt'))
    expected  = Time.parse("2009-08-07 22:52:02")
  ...
  def test_expires_on
    parser    = @klass.new(load_part('registered.txt'))
    expected  = Time.parse("2010-10-03 13:36:48")
