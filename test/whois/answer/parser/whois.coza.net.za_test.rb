...
class AnswerParserWhoisCozaNetZaTest < Whois::Answer::Parser::TestCase

  def setup
    @klass  = Whois::Answer::Parser::WhoisCozaNetZa
    @host   = "whois.coza.net.za"
  end
  ...
  def test_domain
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part('registered.txt')).domain }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part('available.txt')).domain }
  end
