require 'test_helper'
require 'whois/answer/parser/whois.centralnic.com'

class AnswerParserWhoisCentralnicComTest < Whois::Answer::Parser::TestCase

  def setup
    @klass  = Whois::Answer::Parser::WhoisCentralnicCom
    @host   = "whois.centralnic.com"
  end

end

class AnswerParserWhoisCentralnicComAeOrgTest < AnswerParserWhoisCentralnicComTest

  def setup
    super
    @suffix = "ae.org"
  end


  def test_referral_whois
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_whois }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_whois }
  end

  def test_referral_url
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_url }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_url }
  end


  def test_status
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = :registered
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = :available
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end

  def test_available?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = false
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = true
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }
  end

  def test_registered?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = true
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = false
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }
  end


  def test_created_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2010-10-11")
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }
  end

  def test_updated_on
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).updated_on }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).updated_on }
  end

  def test_expires_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2012-10-11")
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }
  end


  def test_nameservers
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = %w( ns1.dmnreseller.com ns2.dmnreseller.com )
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = %w()
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }
  end

end

class AnswerParserWhoisCentralnicComArComTest < AnswerParserWhoisCentralnicComTest

  def setup
    super
    @suffix = "ar.com"
  end


  def test_referral_whois
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_whois }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_whois }
  end

  def test_referral_url
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_url }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_url }
  end


  def test_status
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = :registered
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = :available
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end

  def test_available?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = false
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = true
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }
  end

  def test_registered?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = true
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = false
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }
  end


  def test_created_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2008-04-25")
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }
  end

  def test_updated_on
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).updated_on }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).updated_on }
  end

  def test_expires_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2011-04-25")
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }
  end


  def test_nameservers
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = %w( ns0.centralnic-dns.com ns1.centralnic-dns.com ns2.centralnic-dns.com ns3.centralnic-dns.com ns4.centralnic-dns.com ns5.centralnic-dns.com )
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = %w()
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }
  end

end

class AnswerParserWhoisCentralnicComBrComTest < AnswerParserWhoisCentralnicComTest

  def setup
    super
    @suffix = "br.com"
  end


  def test_referral_whois
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_whois }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_whois }
  end

  def test_referral_url
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_url }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_url }
  end


  def test_status
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = :registered
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = :available
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end

  def test_available?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = false
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = true
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }
  end

  def test_registered?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = true
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = false
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }
  end


  def test_created_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2009-04-17")
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }
  end

  def test_updated_on
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).updated_on }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).updated_on }
  end

  def test_expires_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2011-04-17")
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }
  end


  def test_nameservers
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = %w( ns1.terra.com.br ns2.terra.com.br )
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = %w()
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }
  end

end

class AnswerParserWhoisCentralnicComCnComTest < AnswerParserWhoisCentralnicComTest

  def setup
    super
    @suffix = "cn.com"
  end


  def test_referral_whois
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_whois }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_whois }
  end

  def test_referral_url
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_url }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_url }
  end


  def test_status
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = :registered
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = :available
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end

  def test_available?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = false
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = true
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }
  end

  def test_registered?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = true
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = false
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }
  end


  def test_created_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2005-11-23")
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }
  end

  def test_updated_on
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).updated_on }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).updated_on }
  end

  def test_expires_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2011-11-23")
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }
  end


  def test_nameservers
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = %w( ns1.meteos.it ns2.meteos.it )
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = %w()
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }
  end

end

class AnswerParserWhoisCentralnicComDeComTest < AnswerParserWhoisCentralnicComTest

  def setup
    super
    @suffix = "de.com"
  end


  def test_referral_whois
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_whois }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_whois }
  end

  def test_referral_url
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_url }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_url }
  end


  def test_status
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = :registered
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = :available
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end

  def test_available?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = false
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = true
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }
  end

  def test_registered?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = true
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = false
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }
  end


  def test_created_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2010-03-02")
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }
  end

  def test_updated_on
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).updated_on }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).updated_on }
  end

  def test_expires_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2011-03-02")
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }
  end


  def test_nameservers
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = %w( ns.udagdns.de ns.udagdns.net )
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = %w()
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }
  end

end

class AnswerParserWhoisCentralnicComEuComTest < AnswerParserWhoisCentralnicComTest

  def setup
    super
    @suffix = "eu.com"
  end


  def test_referral_whois
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_whois }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_whois }
  end

  def test_referral_url
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_url }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_url }
  end


  def test_status
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = :registered
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = :available
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end

  def test_available?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = false
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = true
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }
  end

  def test_registered?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = true
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = false
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }
  end


  def test_created_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2001-08-14")
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }
  end

  def test_updated_on
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).updated_on }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).updated_on }
  end

  def test_expires_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2011-08-14")
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }
  end


  def test_nameservers
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = %w( ns1.itransactuk.net ns2.itransactuk.net )
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = %w()
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }
  end

end

class AnswerParserWhoisCentralnicComGbComTest < AnswerParserWhoisCentralnicComTest

  def setup
    super
    @suffix = "gb.com"
  end


  def test_referral_whois
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_whois }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_whois }
  end

  def test_referral_url
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_url }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_url }
  end


  def test_status
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = :registered
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = :available
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end

  def test_available?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = false
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = true
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }
  end

  def test_registered?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = true
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = false
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }
  end


  def test_created_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2006-04-23")
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }
  end

  def test_updated_on
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).updated_on }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).updated_on }
  end

  def test_expires_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2011-04-23")
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }
  end


  def test_nameservers
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = %w( a.ns14.net b.ns14.net c.ns14.net d.ns14.net )
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = %w()
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }
  end

end

class AnswerParserWhoisCentralnicComGbNetTest < AnswerParserWhoisCentralnicComTest

  def setup
    super
    @suffix = "gb.net"
  end


  def test_referral_whois
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_whois }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_whois }
  end

  def test_referral_url
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_url }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_url }
  end


  def test_status
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = :registered
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = :available
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end

  def test_available?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = false
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = true
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }
  end

  def test_registered?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = true
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = false
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }
  end


  def test_created_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2008-12-04")
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }
  end

  def test_updated_on
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).updated_on }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).updated_on }
  end

  def test_expires_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2010-12-04")
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }
  end


  def test_nameservers
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = %w( a.ns14.net b.ns14.net c.ns14.net d.ns14.net )
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = %w()
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }
  end

end

class AnswerParserWhoisCentralnicComHuComTest < AnswerParserWhoisCentralnicComTest

  def setup
    super
    @suffix = "hu.com"
  end


  def test_referral_whois
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_whois }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_whois }
  end

  def test_referral_url
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_url }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_url }
  end


  def test_status
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = :registered
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = :available
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end

  def test_available?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = false
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = true
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }
  end

  def test_registered?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = true
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = false
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }
  end


  def test_created_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2008-08-27")
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }
  end

  def test_updated_on
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).updated_on }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).updated_on }
  end

  def test_expires_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2011-08-27")
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }
  end


  def test_nameservers
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = %w( ns27.worldnic.com ns28.worldnic.com )
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = %w()
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }
  end

end

class AnswerParserWhoisCentralnicComJpnComTest < AnswerParserWhoisCentralnicComTest

  def setup
    super
    @suffix = "jpn.com"
  end


  def test_referral_whois
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_whois }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_whois }
  end

  def test_referral_url
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_url }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_url }
  end


  def test_status
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = :registered
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = :available
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end

  def test_available?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = false
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = true
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }
  end

  def test_registered?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = true
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = false
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }
  end


  def test_created_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2007-06-29")
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }
  end

  def test_updated_on
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).updated_on }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).updated_on }
  end

  def test_expires_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2011-06-29")
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }
  end


  def test_nameservers
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = %w( ns1.diywebbuilder.org ns2.diywebbuilder.org )
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = %w()
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }
  end

end

class AnswerParserWhoisCentralnicComKrComTest < AnswerParserWhoisCentralnicComTest

  def setup
    super
    @suffix = "kr.com"
  end


  def test_referral_whois
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_whois }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_whois }
  end

  def test_referral_url
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_url }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_url }
  end


  def test_status
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = :registered
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = :available
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end

  def test_available?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = false
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = true
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }
  end

  def test_registered?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = true
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = false
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }
  end


  def test_created_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2008-06-11")
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }
  end

  def test_updated_on
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).updated_on }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).updated_on }
  end

  def test_expires_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2011-06-11")
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }
  end


  def test_nameservers
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = %w( ns1.academyart.edu ns2.academyart.edu )
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = %w()
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }
  end

end

class AnswerParserWhoisCentralnicComNoComTest < AnswerParserWhoisCentralnicComTest

  def setup
    super
    @suffix = "no.com"
  end


  def test_referral_whois
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_whois }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_whois }
  end

  def test_referral_url
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_url }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_url }
  end


  def test_status
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = :registered
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = :available
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end

  def test_available?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = false
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = true
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }
  end

  def test_registered?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = true
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = false
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }
  end


  def test_created_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2004-06-07")
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }
  end

  def test_updated_on
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).updated_on }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).updated_on }
  end

  def test_expires_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2012-06-07")
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }
  end


  def test_nameservers
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = %w( ns1.ukdnsservers.co.uk ns2.ukdnsservers.co.uk )
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = %w()
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }
  end

end

class AnswerParserWhoisCentralnicComQcComTest < AnswerParserWhoisCentralnicComTest

  def setup
    super
    @suffix = "qc.com"
  end


  def test_referral_whois
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_whois }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_whois }
  end

  def test_referral_url
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_url }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_url }
  end


  def test_status
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = :registered
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = :available
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end

  def test_available?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = false
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = true
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }
  end

  def test_registered?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = true
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = false
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }
  end


  def test_created_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2004-10-08")
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }
  end

  def test_updated_on
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).updated_on }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).updated_on }
  end

  def test_expires_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2012-10-08")
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }
  end


  def test_nameservers
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = %w( ns1.mydomain.com ns2.mydomain.com ns3.mydomain.com ns4.mydomain.com )
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = %w()
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }
  end

end

class AnswerParserWhoisCentralnicComRuComTest < AnswerParserWhoisCentralnicComTest

  def setup
    super
    @suffix = "ru.com"
  end


  def test_referral_whois
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_whois }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_whois }
  end

  def test_referral_url
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_url }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_url }
  end


  def test_status
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = :registered
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = :available
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end

  def test_available?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = false
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = true
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }
  end

  def test_registered?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = true
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = false
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }
  end


  def test_created_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2006-07-31")
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }
  end

  def test_updated_on
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).updated_on }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).updated_on }
  end

  def test_expires_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2012-07-31")
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }
  end


  def test_nameservers
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = %w( ns12.zoneedit.com ns7.zoneedit.com )
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = %w()
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }
  end

end

class AnswerParserWhoisCentralnicComSaComTest < AnswerParserWhoisCentralnicComTest

  def setup
    super
    @suffix = "sa.com"
  end


  def test_referral_whois
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_whois }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_whois }
  end

  def test_referral_url
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_url }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_url }
  end


  def test_status
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = :registered
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = :available
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end

  def test_available?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = false
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = true
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }
  end

  def test_registered?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = true
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = false
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }
  end


  def test_created_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2007-09-27")
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }
  end

  def test_updated_on
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).updated_on }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).updated_on }
  end

  def test_expires_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2011-09-27")
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }
  end


  def test_nameservers
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = %w( ns1191.websitewelcome.com ns1192.websitewelcome.com )
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = %w()
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }
  end

end

class AnswerParserWhoisCentralnicComSeComTest < AnswerParserWhoisCentralnicComTest

  def setup
    super
    @suffix = "se.com"
  end


  def test_referral_whois
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_whois }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_whois }
  end

  def test_referral_url
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_url }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_url }
  end


  def test_status
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = :registered
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = :available
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end

  def test_available?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = false
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = true
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }
  end

  def test_registered?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = true
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = false
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }
  end


  def test_created_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2008-05-10")
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }
  end

  def test_updated_on
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).updated_on }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).updated_on }
  end

  def test_expires_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2011-05-10")
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }
  end


  def test_nameservers
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = %w( ns1.domaindiscount24.net ns2.domaindiscount24.net ns3.domaindiscount24.net )
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = %w()
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }
  end

end

class AnswerParserWhoisCentralnicComSeNetTest < AnswerParserWhoisCentralnicComTest

  def setup
    super
    @suffix = "se.net"
  end


  def test_referral_whois
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_whois }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_whois }
  end

  def test_referral_url
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_url }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_url }
  end


  def test_status
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = :registered
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = :available
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end

  def test_available?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = false
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = true
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }
  end

  def test_registered?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = true
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = false
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }
  end


  def test_created_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2008-05-10")
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }
  end

  def test_updated_on
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).updated_on }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).updated_on }
  end

  def test_expires_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2011-05-10")
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }
  end


  def test_nameservers
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = %w( ns1.domaindiscount24.net ns2.domaindiscount24.net ns3.domaindiscount24.net )
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = %w()
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }
  end

end

class AnswerParserWhoisCentralnicComUkComTest < AnswerParserWhoisCentralnicComTest

  def setup
    super
    @suffix = "uk.com"
  end


  def test_referral_whois
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_whois }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_whois }
  end

  def test_referral_url
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_url }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_url }
  end


  def test_status
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = :registered
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = :available
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end

  def test_available?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = false
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = true
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }
  end

  def test_registered?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = true
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = false
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }
  end


  def test_created_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2001-08-31")
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }
  end

  def test_updated_on
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).updated_on }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).updated_on }
  end

  def test_expires_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2011-08-31")
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }
  end


  def test_nameservers
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = %w( ns1.myhostcp.com ns2.myhostcp.com )
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = %w()
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }
  end

end

class AnswerParserWhoisCentralnicComUkNetTest < AnswerParserWhoisCentralnicComTest

  def setup
    super
    @suffix = "uk.net"
  end


  def test_referral_whois
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_whois }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_whois }
  end

  def test_referral_url
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_url }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_url }
  end


  def test_status
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = :registered
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = :available
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end

  def test_available?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = false
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = true
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }
  end

  def test_registered?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = true
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = false
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }
  end


  def test_created_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2006-02-28")
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }
  end

  def test_updated_on
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).updated_on }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).updated_on }
  end

  def test_expires_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2012-02-28")
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }
  end


  def test_nameservers
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = %w( ns1.myhostcp.com ns2.myhostcp.com )
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = %w()
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }
  end

end

class AnswerParserWhoisCentralnicComUsComTest < AnswerParserWhoisCentralnicComTest

  def setup
    super
    @suffix = "us.com"
  end


  def test_referral_whois
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_whois }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_whois }
  end

  def test_referral_url
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_url }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_url }
  end


  def test_status
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = :registered
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = :available
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end

  def test_available?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = false
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = true
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }
  end

  def test_registered?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = true
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = false
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }
  end


  def test_created_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2003-10-20")
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }
  end

  def test_updated_on
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).updated_on }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).updated_on }
  end

  def test_expires_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2014-10-20")
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }
  end


  def test_nameservers
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = %w( ns1.p17.dynect.net ns2.p17.dynect.net ns3.p17.dynect.net ns4.p17.dynect.net )
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = %w()
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }
  end

end

class AnswerParserWhoisCentralnicComUyComTest < AnswerParserWhoisCentralnicComTest

  def setup
    super
    @suffix = "uy.com"
  end


  def test_referral_whois
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_whois }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_whois }
  end

  def test_referral_url
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_url }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_url }
  end


  def test_status
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = :registered
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = :available
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end

  def test_available?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = false
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = true
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }
  end

  def test_registered?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = true
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = false
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }
  end


  def test_created_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2009-05-04")
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }
  end

  def test_updated_on
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).updated_on }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).updated_on }
  end

  def test_expires_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2011-05-04")
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }
  end


  def test_nameservers
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = %w( ns79.worldnic.com ns80.worldnic.com )
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = %w()
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }
  end

end

class AnswerParserWhoisCentralnicComZaComTest < AnswerParserWhoisCentralnicComTest

  def setup
    super
    @suffix = "za.com"
  end


  def test_referral_whois
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_whois }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_whois }
  end

  def test_referral_url
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).referral_url }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).referral_url }
  end


  def test_status
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = :registered
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = :available
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end

  def test_available?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = false
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = true
    assert_equal  expected, parser.available?
    assert_equal  expected, parser.instance_eval { @available }
  end

  def test_registered?
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = true
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = false
    assert_equal  expected, parser.registered?
    assert_equal  expected, parser.instance_eval { @registered }
  end


  def test_created_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2004-11-17")
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.created_on
    assert_equal  expected, parser.instance_eval { @created_on }
  end

  def test_updated_on
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/registered.txt")).updated_on }
    assert_raise(Whois::PropertyNotSupported) { @klass.new(load_part("/#{@suffix}/available.txt")).updated_on }
  end

  def test_expires_on
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = Time.parse("2013-11-17")
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = nil
    assert_equal  expected, parser.expires_on
    assert_equal  expected, parser.instance_eval { @expires_on }
  end


  def test_nameservers
    parser    = @klass.new(load_part("/#{@suffix}/registered.txt"))
    expected  = %w( ns1a.your-server.co.za nsa.second-ns.co.za )
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }

    parser    = @klass.new(load_part("/#{@suffix}/available.txt"))
    expected  = %w()
    assert_equal  expected, parser.nameservers
    assert_equal  expected, parser.instance_eval { @nameservers }
  end

end
