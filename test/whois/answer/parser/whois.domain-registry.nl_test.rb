...
class AnswerParserWhoisDomainRegistryNlTest < Whois::Answer::Parser::TestCase
  ...
  def setup
    @klass  = Whois::Answer::Parser::WhoisDomainRegistryNl
  ...
  def test_status
    ...
    parser    = @klass.new(load_part('/quarantine.txt'))
    expected  = :quarantine
    assert_equal  expected, parser.status
    assert_equal  expected, parser.instance_eval { @status }
  end
  ...
  def test_created_on
    assert_equal  Time.parse("1999-05-27"),
  ...
  def test_updated_on
    assert_equal  Time.parse("2009-02-11"),
