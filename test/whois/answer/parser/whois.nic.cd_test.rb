require 'test_helper'
require 'whois/answer/parser/whois.nic.cd'
...
class AnswerParserWhoisNicCdTest < Whois::Answer::Parser::TestCase
  ...
  def test_status
    ...
    expected  = ["CLIENT TRANSFER PROHIBITED", "CLIENT DELETE PROHIBITED"]
  ...
  def test_created_on
    parser    = @klass.new(load_part('registered.txt'))
    expected  = Time.parse("2006-09-30 22:26:42")
  ...
  def test_expires_on
    parser    = @klass.new(load_part('registered.txt'))
    expected  = Time.parse("2010-10-01 00:00:00")
